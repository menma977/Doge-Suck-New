...
class FibonacciActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        suck.setOnClickListener {
            ...
            if (percent.text.isEmpty()) {
                ...
                basePayInValue =
                    (Config().getLabor(balanceTextView.text.toString().toDouble() * 0.00000001) * 10).toBigDecimal()
                targetBalanceValue =
                    balanceTextView.text.toString().toDouble() + ((balanceTextView.text.toString().toDouble() * percent.text.toString().toInt()) / 100)
    ...
    private fun bot() {
        ...
        Timer().schedule(1000, 1000) {
            ...
            payIn =
                (basePayInValue * fibonacciArray[jumper].toBigDecimal()) * (100000000).toBigDecimal()
            ...
            runOnUiThread {
                try {
                    val payInResponse = (-basePayInValue.toDouble())
