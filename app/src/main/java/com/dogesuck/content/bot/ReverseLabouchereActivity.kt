...
class ReverseLabouchereActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_reverse_labouchere)
    ...
    private fun bot() {
        ...
        var low: Int
        var high: Int
        var highBalance: BigDecimal = (0).toBigDecimal()
        payInDefault = payIn
        Timer().schedule(1000, 1000) {
            try {
                ...
                runOnUiThread {
                    ...
                    if ((balance.text.toString().toBigDecimal() * (0.00000001).toBigDecimal()) >= (highBalance * (0.00000001).toBigDecimal())) {
                        labour.clear()
                        payIn = payInDefault
                        labour.add(formatDouble.format(payIn).toDouble())
                        highBalance = balance.text.toString().toBigDecimal()
                    } else {
                        if (payInWin) {
                    ...
                    payInWin = responseProfit <= (0).toBigDecimal()
